GLOBALS_SECTION;
  ...
  const double LOG_M_PI = log(M_PI);
   ...
   //  issams7 -noinit -iprint 1 -shess &> issams7.out&
   //  issams7 -noinit -mcmc2 500000 -mcsave 20 -shess &> issams7.out
...
DATA_SECTION
  ...
  init_int phase_sdlogN;
  init_number init_sdlogN;
  !! TTRACE(init_sdlogN,phase_sdlogN)

  init_int phase_sdlogI;
  init_number init_sdlogI;
  !! TTRACE(init_sdlogI,phase_sdlogI)

  init_int phase_sdlogFmort;
  init_number init_sdlogFmort;
  !! TTRACE(init_sdlogFmort,phase_sdlogFmort)
...
PARAMETER_SECTION
  ...
  // general biomass process error standard deviations
  // init_number logsdlogBProc(phase_sdlogBProc);

  // biomass process error standard deviation
  init_number logsdlogN(phase_sdlogN);

  // abundance index process error standard deviation
  init_number logsdlogI(phase_sdlogI);

  // fishing mortality process error standard deviation
  init_number logsdlogFmort(phase_sdlogFmort);
  ...
  sdreport_number ar;
  sdreport_number aK;
  sdreport_number asdlogN;
  sdreport_number asdlogI;
...
PRELIMINARY_CALCS_SECTION
    ...
    if (!pininit)
    {
       ...
       logsdlogN = log(init_sdlogN);
       TTRACE(logsdlogN,log(init_sdlogN))
       logsdlogI = log(init_sdlogI);
       TTRACE(logsdlogI,log(init_sdlogI))
...
PROCEDURE_SECTION
  ...
  step0(U(utPop+1), logsdlogN, logFmsy, logMSY, logitQ, logsdlogI);
 
  for (int t = 2; t <= ntime; t++)
  {
     step(t, U(Fndxl(t-1),Fndxu(t-1)), U(Fndxl(t),Fndxu(t)),
             U(utPop+t-1), U(utPop+t), logsdlogN, logsdlogFmort,
             logFmsy, logMSY, logitQ, logsdlogI);
  }
  ...
  aMSY = mfexp(logMSY);
  alogMSY = logMSY;
  asdlogN = mfexp(logsdlogN);
  asdlogI = mfexp(logsdlogI);
...
SEPARABLE_FUNCTION void step0(const dvariable& p11, const dvariable& lsdlogN, const dvariable& lFmsy, const dvariable& lMSY, const dvariable& lgtQ, const dvariable& lsdlogI)
  ...
  if (use_Q)
  {
     dvariable varlogI = square(mfexp(lsdlogI));
     dvariable lnQ=log(alogit((dvariable)lgtQ));
     dvariable logib = lnQ + log(immigrant_biomass(1));
     Qnll += 0.5*(log(TWO_M_PI*varlogI) + square(logib-p11)/varlogI);
     nll += Qnll;
  }
  NLL_TRACE(Qnll)
  NLL_TRACE(nll)
...
SEPARABLE_FUNCTION void step(const int t, const dvar_vector& f1, const dvar_vector& f2, const dvariable& p11, const dvariable p12, const dvariable& lsdlogN, const dvariable& lsdlogFmort, const dvariable& lFmsy, const dvariable& lMSY, const dvariable& lgtQ, const dvariable& lsdlogI)
  ...
  dvariable lsdlogF = lsdlogFmort;
  dvariable varlogF = square(mfexp(lsdlogF));
  dvariable lsdlogPop = lsdlogN;
  dvariable varlogPop = square(mfexp(lsdlogN));
  ...
  if (use_Q)
  {
     ...
     Qnll += 0.5*(log(TWO_M_PI*varlogI) + square(logib-nextLogN)/varlogI);
     nll += Qnll;
  }
  NLL_TRACE(Qnll)
  NLL_TRACE(nll)
...
FUNCTION void write_status(ofstream& s)
    ...
    s << "#    K = " << K << endl;
    s << "#   logsdlogN: " << logsdlogN
             <<  " (" << active(logsdlogN) <<")" << endl;
    s << "#      sdlogN: " << mfexp(logsdlogN) << endl;
    s << "#   logsdlogI: " << logsdlogI
             <<  " (" << active(logsdlogI) <<")" << endl;
    s << "#      sdlogI: " << mfexp(logsdlogI) << endl;
    s << "#   logsdlogFmort: " << logsdlogFmort
             <<  " (" << active(logsdlogFmort) <<")" << endl;
    s << "#      sdlogFmort: " << mfexp(logsdlogFmort) << endl;
