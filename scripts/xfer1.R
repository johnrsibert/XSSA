...
plot.logistic.xfer=function(r=0.5,K=1.0,F1=r/2,aT12=0.0,aT21=0.0,sT=0.0,dt=0.5,ntime=100)
{

   main = paste("F=",F1,", T12=",aT12,", T21=",aT21,", sT=",sT,sep="")
   ss = 1.0/dt
   ...
   res = logistic.xfer(r,K,F1,aT12,aT21,sT,dt,ntime)
   ...
   abline(h=K,lty="dotdash")
   abline(h=Nmsy,lty="dotdash")
   abline(h=MSY,lty="dotdash",col="red")
   abline(h=0.9,lty="dotdash",col="green")

   file = paste("r",r,"F",F1,"T12",aT12,"T21",aT21,"sT",sT,sep="")
   print(file)
...
draw.examples=function()
{
   ...
   x11()
   plot.logistic.xfer(aT21=0.1)->tmp
   x11()
   plot.logistic.xfer(aT21=0.01)->tmp
   x11()
   plot.logistic.xfer(aT21=0.001)->tmp
   x11()
   plot.logistic.xfer(aT12=0.1)->tmp
   x11()
   plot.logistic.xfer(aT12=0.01)->tmp
   x11()
   plot.logistic.xfer(aT12=0.001)->tmp
   x11()
   plot.logistic.xfer(aT21=0.001,aT12=0.01)->tmp

}

logistic.xfer=function(r=0.5,K=1.0,F1=0.05,aT12=0.0,aT21=0.0,sT=0.0,dt=5,ntime=100)
{
   ...
   ss = 1.0/dt
   p = rnorm(n=ntime,sd=sT)
   q = rnorm(n=ntime,sd=sT)
